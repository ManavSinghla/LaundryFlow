...
const NewOrder = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let customerId;
      try {
        const { data: newCustomer } = await axios.post('http://localhost:5000/api/customers', customer, config);
        customerId = newCustomer._id;
      } catch (err) {
        // If exists, find them by phone
        if (err.response?.status === 400) {
           const { data: existing } = await axios.get(`http://localhost:5000/api/customers?phone=${customer.phone}`, config);
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" name="phone" required value={customer.phone} onChange={handleCustomerChange} className="input-field" placeholder="1234567890" />
            </div>
