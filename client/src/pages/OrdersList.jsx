...
const OrdersList = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (filter === 'Pending') filteredData = data.filter(o => o.status !== 'Delivered');
      if (filter === 'Delivered') filteredData = data.filter(o => o.status === 'Delivered');
  ...
  const handlePaymentChange = async (id, newPaymentStatus) => {
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await axios.put(`http://localhost:5000/api/orders/${id}/payment`, { paymentStatus: newPaymentStatus }, config);
      fetchOrders();
    } catch (error) {
        ...
        <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg border border-gray-200 dark:border-gray-700">
          {['All', 'Pending', 'Delivered'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${filter === f ? 'bg-white dark:bg-gray-700 shadow flex-1 text-primary-600' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 flex-1'}`}
            >
              {f}
            </button>
          ))}
